...
for (const [row_index, row] of matrix.entries()) {
	...
	for (const [cell_index, cell] of row.entries()) {
		...
		cell.addEventListener("click", event => {
			...
			cell.style.backgroundColor = "rgb(" + generateNumber() + ',' + generateNumber() + ',' + generateNumber() + ')';
			...
			for (const row_to_paint of matrix) { // pega cada item da matrix
				// pega o indice igual ao cell_index do item da matrix
